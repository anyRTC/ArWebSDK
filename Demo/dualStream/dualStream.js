var client = ArRTC.createClient({ mode: "rtc", codec: "h264" });
...
async function join() {
  ...
  client.on("stream-type-changed", handleUserStreamTypeChanged);
...
async function setSomeUserHQStream(HQStreamUserList = []){
  ...
  const allUserList = [...Object.keys(remoteUsers)]
  ...
  LQStreamUserList.forEach(user=> void handlePromiseList.push(
    async () => {
      console.log(`set LQStreamUserList user: ${user} to LQ Stream`);
  ...
  HQStreamUserList.forEach(user=> void handlePromiseList.push(
    async ()=> {
      console.log(`set HQStreamUserList user: ${user} to HQ Stream`);
...
function handleUserStreamTypeChanged(uid, streamType) {
